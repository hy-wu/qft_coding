...
| **单原子链色散** | $\omega(k) = 2\sqrt{\frac{C}{M}}\left\vert\sin\frac{ka}{2}\right\vert$ | [phonon_01_dispersion_modes.py](phonon_simulation/phonon_01_dispersion_modes.py) | `dispersion_monoatomic()` |
...
| **2D 态密度 & 鞍点** | $g(\omega) = \int \frac{dS}{(2\pi)^d \mid\nabla_{\mathbf{k}} \omega\mid}$ | [phonon_02_dos_van_hove.py](phonon_simulation/phonon_02_dos_van_hove.py) | `compute_2d_dispersion_and_dos()` 采用 2D 布里渊区网格采样 `histogram` 计算对数奇点 |
